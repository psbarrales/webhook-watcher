...
type LoggerArguments = [string, string, string, number, string, string]
...
const transporter = (str: string, args: LoggerArguments) => {
  if (!args[3] && !args[4]) {
    metrics.counter({ name: 'request_total', help: 'Total HTTP requests' }, 'inc')
  }
  console.info(str, {
    method: args[1],
    url: args[2],
